...
def head_hunter_vacancies_information(data: list) -> list:
    ...
    for stack in data:
        ...
        experience = stack.get("experience", {}).get("name", "Без опыта")
        alternate_url = stack.get("alternate_url", "Ссылка отсутствует")
        currency = stack.get("salary", {}).get("currency", "Не указано") if stack.get("salary") else "Не указано"
